...
import { CronosDate, CronosTimezone } from "./date.js";
import { type DateSequence } from "./scheduler.js";
import { flatMap } from "./utils.js";

const hourinms = 60 * 60 * 1000;
const findFirstFrom = (from: number, list: number[]) =>
  list.findIndex((n) => n >= from);
...
export class CronosExpression implements DateSequence {
  ...
  static parse(
    ...
    if (options.strict) {
      ...
      if (typeof options.strict === "object") {
        warnings = warnings.filter(
          (warning) =>
            !!(options.strict as { [key in WarningType]?: boolean })[
              warning.type
            ]
        );
      }
      if (warnings.length > 0) {
        throw new Error(
          `Strict mode: Parsing failed with ${warnings.length} warnings`
        );
      }
    }
    ...
    expr.timezone =
      options.timezone instanceof CronosTimezone
        ? options.timezone
        : options.timezone !== undefined
        ? new CronosTimezone(options.timezone)
        : undefined;
    expr.skipRepeatedHour =
      options.skipRepeatedHour !== undefined
  ...
  nextDate(afterDate: Date = new Date()): Date | null {
    ...
    const fromTimestamp = afterDate.getTime(),
      ...
      prevHourLocalTimestamp = CronosDate.fromDate(
        new Date(fromTimestamp - hourinms),
        this.timezone
      )["toUTCTimestamp"](),
      nextHourLocalTimestamp = CronosDate.fromDate(
        new Date(fromTimestamp + hourinms),
        this.timezone
      )["toUTCTimestamp"](),
      ...
      thisHourMissing =
        fromLocalTimestamp - prevHourLocalTimestamp === hourinms * 2;

    if (this.skipRepeatedHour && thisHourRepeated) {
      return this._next(
        fromCronosDate.copyWith({ minute: 59, second: 60 }),
        false
      ).date;
    }
    if (this.missingHour === "offset" && thisHourMissing) {
      const nextDate = this._next(
        fromCronosDate.copyWith({ hour: fromCronosDate.hour - 1 })
      ).date;
      if (!nextDate || nextDate.getTime() > fromTimestamp) return nextDate;
    }

    let { date: nextDate, cronosDate: nextCronosDate } =
      this._next(fromCronosDate);

    if (this.missingHour !== "offset" && nextCronosDate && nextDate) {
      const nextDateNextHourTimestamp = nextCronosDate
        .copyWith({ hour: nextCronosDate.hour + 1 })
        .toDate(this.timezone)
        .getTime();
      if (nextDateNextHourTimestamp === nextDate.getTime()) {
        if (this.missingHour === "insert") {
          return nextCronosDate
            .copyWith({ minute: 0, second: 0 })
            .toDate(this.timezone);
        }
        // this.missingHour === 'skip'
        return this._next(nextCronosDate.copyWith({ minute: 59, second: 59 }))
          .date;
      }
    }
    ...
    if (!this.skipRepeatedHour) {
      if (
        nextHourRepeated &&
        (!nextDate || nextDate.getTime() > fromTimestamp + hourinms)
      ) {
        nextDate = this._next(
          fromCronosDate.copyWith({ minute: 0, second: 0 }),
          false
        ).date;
      }
  ...
  private _nextMonth(fromDate: CronosDate): CronosDate | null {
    ...
    while (!nextDate) {
      ...
      nextDate = this._nextDay(
        nextMonth === fromDate.month
          ? fromDate
          : new CronosDate(fromDate.year, nextMonth)
      );
  ...
  private _nextDay(fromDate: CronosDate): CronosDate | null {
    ...
    while (!nextDate) {
      ...
      nextDate = this._nextHour(
        nextDay === fromDate.day
          ? fromDate
          : new CronosDate(fromDate.year, fromDate.month, nextDay)
      );
