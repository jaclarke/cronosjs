...
function addTask(task: CronosTask) {
  if (task["_timestamp"] !== undefined) {
    const insertIndex = scheduledTasks.findIndex(
      (t) => t["_timestamp"]! < task["_timestamp"]!
    );
...
function runScheduledTasks(skipRun = false) {
  ...
  const removeIndex = scheduledTasks.findIndex(
    (task) => task["_timestamp"]! <= now
  );
...
export class CronosTask {
  ...
  constructor(sequenceOrDates: DateSequence | DateLike[] | DateLike) {
  ...
  on<K extends keyof CronosTaskListeners>(
    event: K,
    listener: CronosTaskListeners[K]
  ) {
    this._listeners[event].add(listener as any);
    return this;
  }

  off<K extends keyof CronosTaskListeners>(
    event: K,
    listener: CronosTaskListeners[K]
  ) {
